...
import { faCheckDouble, faPaperPlane, faSearch } from "@fortawesome/free-solid-svg-icons";
...
const date_OF_lAST_Message=(SenderId)=>{
  const user_notification=notifications.filter(notf=>notf.senderId === SenderId);
...
const Filter_Notification = (senderId)=>{
  if(notifications.length >= 1){
    const filter_Notification =notifications.filter(item=> item.senderId !== senderId)
...
////////////////////////////////////////////////////
  ...
  const Count_Messages=(senderId)=>{
    // let count=0;
    const user_notification=notifications.filter(item=>item.senderId == senderId && item.isRead === false);
...
/////////////////////////////////// for the length of the last message send showed in chat div 
  ...
  return (
    <main className=" flex flex-row  h-screen   " style={{ overflow: "hidden" }}>
      <div className=" flex flex-col bg-gray-800 h-screen   basis-[30%]   h-full    border-solid  border-r-4 border-gray-700 shadow-2xl " >
        ...
        { loadingChat && filter_chat?.length >=1 ? (
          filter_chat.map((item,index) => {
            return (
              <div className="pl-2 pr-2  " key={index}>
                <div
                  ...
                  onClick={() => {
                    chat_box_info(item);Filter_Notification(item._id)
                  }}
                >  
            ...
            <img 
                ...
                />
                  ...
                  {onlineUser && onlineUser.some(user=>user.userId === item._id)?<div className="rounded-full animate-pulse   w-3 h-2.5 bg-green-600" style={{position:"relative" , right:"2%" ,bottom:'1%'}}></div>:<div className="rounded-full  w-3 h-2.5 " style={{position:"relative" , right:"2%" ,bottom:'1%'}}></div>}
                  <div className=" w-full">
                    <span className="flex flex-row " style={{ width: "100%" }}>
                      ...
                      <p className="text-white  text-xs basis-1/2 text-right ">
                        {date_OF_lAST_Message(item._id) ?moment(date_OF_lAST_Message(item._id || item.SendAt)).calendar():moment(item.last_message?.createdAt).calendar()} 
                    
                      </p>
                    </span>
                    <span className=" flex flex-row">
                      <p
                        className={`${Count_Messages(item._id)? 'text-white':' text-gray-300'} pl-2 col-span-4  w-full  `}
                        style={{ width: "100%" }}>
         ...
         <div className="text-white text-center  mt-[10%]"><h1><br /><br />
        No chat found ,create your first chat...! </h1> <button className="bg-blue-600 hover:bg-blue-400 rounded-lg w-40 h-10 mt-10 text-center ">Create Chat</button></div>
        
      :<>

      <div className="flex flex-nowrap mt-2">  
     <span> <Skeleton variant="circular" width={70} height={70}   sx={{marginLeft:"20px "}}  /></span>
    <span className="w-full mr-4"><Skeleton  variant="rounded" width={"100%"}  height={70} sx={{marginRight:"10px ",marginLeft:"5px",marginTop:"6px"}}  /></span>
    </div>
     <div className="flex flex-nowrap mt-2">  
     <span> <Skeleton variant="circular" width={70} height={70}  sx={{marginLeft:"20px "}}  /></span>
    <span className="w-full mr-4 "><Skeleton  variant="rounded" width={"100%"}  height={70} sx={{marginRight:"10px ",marginLeft:"5px",marginTop:"6px"}}  /></span>
