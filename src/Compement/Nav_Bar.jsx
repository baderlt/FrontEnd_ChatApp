...
export default function Nav_Bar() {
  ...
  const Notifications_ = notifications.filter(
    (item) =>
      item.isRead !== true &&
      !uniqueId.has(item.senderId) &&
      uniqueId.add(item.senderId)
  );
                        ...
                        ? Notifications_.map((item) => {
                            return (
                              ...
                              >
                                {" "}
                                <div
                                  className="w-full text-gray-200 flex flex-row "
                                  style={{ width: "250px" }}
                                >
                                  {" "}
                                  <strong>
                                    {Get_Name(item.senderId)}{" "}
                                  </strong>{" "}
                                  :&ensp;{" "}
                                  <p className="text-white  ">
                                    {Count_Messages(item.senderId)}
                                  </p>{" "}
                                </div>
                                <p
                                  className="text-right text-gray-300 -mb-2 "
                                  style={{ fontSize: 10 }}
                                >
                                  {moment(item.date || item.SendAt).calendar()}{" "}
                                </p>
                              </Alert>
                            );
                          })
